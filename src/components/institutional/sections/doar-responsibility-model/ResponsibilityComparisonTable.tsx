...
/* Mobile: card layout */
function MobileCard({ func }: { func: (typeof RESPONSIBILITY_MODEL_FUNCTIONS)[number] }) {
    const Icon = FUNCTION_ICONS[func.icon];

    return (
        <div className="rounded-xl border border-white/8 bg-deep-space/80 p-4">
            <div className="flex items-center gap-3">
                <Icon className="h-5 w-5 shrink-0 text-doar-blue" strokeWidth={1.75} aria-hidden="true" />
                <span className="text-sm font-medium text-soft-gray">{func.function}</span>
            </div>
            <div className="mt-3 flex items-center gap-6 border-t border-white/8 pt-3">
                <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-doar-blue">DOAR</span>
                    {func.doar && <CheckBadge variant="blue" aria-label="Responsabilidad de DOAR" />}
                </div>
                <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-doar-gold">Partners</span>
                    {func.partners && <CheckBadge variant="gold" aria-label="Responsabilidad de Partners" />}
                </div>
            </div>
        </div>
    );
...
export default function ResponsibilityComparisonTable() {
    return (
        <>
            {/* Mobile cards */}
            <div className="flex flex-col gap-3 lg:hidden" role="list" aria-label="Tabla comparativa de responsabilidades">
                {RESPONSIBILITY_MODEL_FUNCTIONS.map((func) => (
                    <MobileCard key={func.id} func={func} />
                ))}
            </div>

            {/* Desktop table */}
            <div
                className="hidden overflow-x-auto rounded-2xl border border-white/8 bg-deep-space/80 lg:block"
                role="region"
