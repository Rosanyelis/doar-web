...
import { LEDGER_FIRST_CAPABILITIES } from '../../../../constants/institutional';
import { cn } from '../../../../lib/utils';

const CAPABILITY_ICONS: Record<(typeof LEDGER_FIRST_CAPABILITIES)[number]['id'], LucideIcon> = {
    balances: Wallet,
    actividad: Activity,
    historial: Contact,
    estados: RefreshCw,
    movimientos: ArrowLeftRight,
    conciliacion: ShieldCheck,
};

export default function LedgerCapabilityGrid() {
    return (
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
            {LEDGER_FIRST_CAPABILITIES.map(({ id, title, description }) => {
